async function fetchPosts() {
  ...
  if (!res.ok) {
    ...
    document.getElementById("posts").innerHTML =
      "<p class='text-red-500'>Error loading posts. Please try again later.</p>";
    return;
  }
  ...
  likeBtns.forEach((btn_span) => {
    btn_span.addEventListener("click", async () => {
      ...
      if (!svgIcon || !likeCntSpanElement) {
        console.error(
          "Could not find SVG icon or like count span for post:",
          postId
        );
      ...
      const newOptimisticLikeCount = newOptimisticLikedState
        ? originalLikeCount + 1
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (
            responseJson.new_like_count !== newOptimisticLikeCount ||
            responseJson.liked_by_user !== newOptimisticLikedState
          ) {
            console.warn(
              "Server state differs from optimistic update. Syncing UI with server."
            );
            likeCntSpanElement.textContent = responseJson.new_like_count;
            svgIcon.setAttribute(
              "fill",
              responseJson.liked_by_user ? "black" : "grey"
            );
          }
        } else {
